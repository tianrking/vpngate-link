...
4. 需要回到完整节点列表时点击 **Reset Filters**。
5. 选择一个可达服务器并连接。后端会启动 OpenVPN，隧道就绪后才会标记为 active。
6. 在网页里查看当前出口 IP。
7. 把 `127.0.0.1:19080` 配成你的 mieru/hysteria2/sing-box 出站代理。
...
| Web 管理台 | 已实现 | token 状态、节点刷新、多维筛选、重置筛选、批量可达性扫描、连接、断开、收藏、设置、日志、健康检查、出口 IP 检查。 |
